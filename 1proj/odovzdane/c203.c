...
int nextIndex( int index ) {
	return ((++index) % QUEUE_SIZE); //TODO kontrola
}
...
int Queue_IsEmpty( const Queue *queue ) {
	return (queue->firstIndex == queue->freeIndex) ? 1 : 0; //TODO kontrola
}
...
int Queue_IsFull( const Queue *queue ) {
	return (nextIndex(queue->freeIndex) == queue->firstIndex) ? 1 : 0 ; //TODO kontrola
}
