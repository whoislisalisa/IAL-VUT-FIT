 ...
 **/
void List_Dispose( List *list ) { //nastavy prvy prvok aktivnym a postupne posuva prvky
	ListElementPtr nextElement = NULL;
...
void List_InsertFirst( List *list, int data ) {
	ListElementPtr element = malloc(sizeof(struct ListElement)); //alokacia pamate
	if (element == NULL) //chyba pri nedostatku pamate
	{
	    List_Error();
	}
	else //priradi prvok s hodnotou do zoznamu na zaciatok
	{
		element->data = data;
 ...
 * @param list Ukazatel na inicializovanou strukturu jednosměrně vázaného seznamu
 */
void List_DeleteFirst( List *list ) { //rusi prvy prvok zoznamu a aj ak bol aktivny predava aktivitu nasledujucemu
	ListElementPtr firstElement = list->firstElement;
	if (list->firstElement != NULL) 
	{
		list->firstElement = firstElement->nextElement;

		if (firstElement == list->activeElement)
		{
			list->activeElement = NULL;
		}
		free(firstElement); //uvolni pouzivanu pamat
	}
}
...
void List_DeleteAfter( List *list ) {
	if (list->activeElement != NULL && list->activeElement->nextElement != NULL)
	{
		...
		free(element); //uvolni pouzivanu pamat
	}
}
...
void List_InsertAfter( List *list, int data ) {
	...
	if(list->activeElement != NULL)
	{
		...
		else //priradzovanie hodnoty a ukazatelov 
		{
			element->data = data;
...
void List_GetValue( List *list, int *dataPtr ) {
    if(list->activeElement == NULL) //chyba pri aktivite
	{
		List_Error();
	}
	if(list->activeElement != NULL)
...
void List_SetValue( List *list, int data ) {
	if(list->activeElement != NULL) //ak nie je zoznam prázdny
	{
		list->activeElement->data = data; //prepíše dáta v aktívnom prvku zoznamu na dáta
	}
}
...
int List_IsActive( List *list ) {
	return(list->activeElement == NULL ? 0 : 1); //prázdny zoznam vracia 0, neprázdny 1
}
