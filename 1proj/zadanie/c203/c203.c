...
void Queue_Front( const Queue *queue, char *dataPtr ) {
    if(Queue_IsEmpty(queue)) //chybova hlaska
    {
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	if(Queue_IsEmpty(queue)) //chybova hlaska pri prazdnej fronte
	{
 ...
 */
void Queue_Enqueue( Queue *queue, char data ) {	//zistovanie ci je fronta plna a nasledne vkladanie dat  do fronty
	if(Queue_IsFull(queue)) //chybova hlaska pri plnej fronte
	{
